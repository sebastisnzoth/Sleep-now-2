...
function getAdminDb() {
  ...
  const databaseId =
    ...
    "ai-studio-insomnia0-353ab32a-f0f0-410d-9d48-f0543b46d89f";
...
export default async function handler(req: HotmartRequest, res: HotmartResponse) {
  ...
  const validHottok =
    receivedHottok.length === configuredHottok.length &&
    crypto.timingSafeEqual(Buffer.from(receivedHottok), Buffer.from(configuredHottok));
  ...
  if (!buyerEmail) {
    console.warn("Hotmart webhook missing buyer email", { event, transaction });
  ...
  try {
    ...
    await db.runTransaction(async (tx) => {
      ...
      tx.set(
        entitlementRef,
        {
          ...
          ...(accessAction === "grant"
            ? { grantedAt: FieldValue.serverTimestamp(), revokedAt: null }
    ...
    console.info("Hotmart premium access synchronized", {
      event,
      transaction,
      accessAction,
      entitlementId,
    ...
    console.error("Failed to synchronize Hotmart access", {
      event,
      transaction,
